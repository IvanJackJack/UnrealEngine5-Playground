...
void AReidsCharacter::ApplyMovement() {
	if(currentCharacterMovement.Size() != 0.f) {
		AddMovementInput(characterDirection, inputValuesMove.Size());
	}
}
...
void AReidsCharacter::EndWallrun(EReidsWallrunEndreason endreason) {
	// UCustomUtils::Print("Wallrun end");

	switch (endreason) {
	case EReidsWallrunEndreason::Jump:
		ResetJump(jumpsMax - 1);
		break;
	case EReidsWallrunEndreason::Fall:
		ResetJump(1);
		break;
	}
...
void AReidsCharacter::FindRunDirectionAndSide(FVector wallNormal) {
	...
	if(dotSide>=0.f) {
		wallrunSide=EReidsWallrunSide::Right;
		localDirection=FVector::UpVector;
	}else {
		wallrunSide=EReidsWallrunSide::Left;
...
FVector AReidsCharacter::FindLaunchVelocity() {
	...
	if(bIsWallrunning) {
		...
		switch (wallrunSide) {
			case EReidsWallrunSide::Left:
				wallCrossDir=FVector(0.f, 0.f, 1.f);
				break;
			case EReidsWallrunSide::Right:
				wallCrossDir=FVector(0.f, 0.f, -1.f);
				break;
		}
...
void AReidsCharacter::UpdateWallrun() {
	if(!AreRequiredKeysDown()) {
		EndWallrun(EReidsWallrunEndreason::Fall);
		return;
	}

	FVector vecToWall;
	if(wallrunSide==EReidsWallrunSide::Right) {
		...
		) 
	{
		...
		EReidsWallrunSide lastSide=wallrunSide;
		FindRunDirectionAndSide(Hit.ImpactNormal);
